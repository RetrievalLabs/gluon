...
class HarnessPathsTests(unittest.TestCase):
    def test_fixed_output_paths(self) -> None:
        ...
        self.assertEqual(
            ...
            Path(
                "/opt/gluon/org/model-classification-report/project/model-classification-report.json"
            ),
        )
        self.assertEqual(
            paths.model_classification_report_output_dir,
            Path("/opt/gluon/org/model-classification-report"),
        )
        self.assertEqual(
            paths.configuration_classification_report,
            Path(
                "/opt/gluon/org/configuration-classification-report/project/configuration-classification-report.json"
            ),
        )
        self.assertEqual(
            paths.configuration_classification_report_output_dir,
            Path("/opt/gluon/org/configuration-classification-report"),
        )
