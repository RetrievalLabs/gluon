...
class ClaudeAgentTests(unittest.TestCase):
    ...
    def test_characterization_prompt_orders_agent_handoff(self) -> None:
        ...
        self.assertIn("self-contained request/response handoff", prompt)
        self.assertIn("do not rely on live subagent memory", prompt)
        self.assertIn("prior JSON and current scenario state", prompt)
