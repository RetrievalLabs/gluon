...
mod tests {
    ...
    #[test]
    fn skips_tests_integration_tests_and_generated_sources() {
        let root = test_dir("business-skip-non-production");
        fs::create_dir_all(root.join("src/main/java/demo")).unwrap();
        fs::create_dir_all(root.join("src/test/java/demo")).unwrap();
        fs::create_dir_all(root.join("src/integrationTest/java/demo")).unwrap();
        fs::create_dir_all(root.join("src/generated/java/demo")).unwrap();
        fs::create_dir_all(root.join("acceptance-test/src/acceptance-test/java/demo")).unwrap();
        fs::write(
        ...
        .unwrap();
        fs::write(
            root.join("src/main/java/demo/PaymentServiceTest.java"),
            "package demo; class PaymentServiceTest { void test() {} }",
        )
        .unwrap();
        fs::write(
            root.join("src/main/java/demo/GeneratedClient.java"),
            "package demo; @Generated class GeneratedClient { void call() {} }",
        )
        .unwrap();
        fs::write(
            root.join("src/test/java/demo/OrderServiceTest.java"),
            "package demo; class OrderServiceTest { void approve() {} }",
        )
        .unwrap();
        fs::write(
            root.join("src/integrationTest/java/demo/OrderServiceIT.java"),
            "package demo; class OrderServiceIT { void approve() {} }",
        )
        .unwrap();
        fs::write(
            root.join("src/generated/java/demo/GeneratedOrder.java"),
            "package demo; class GeneratedOrder { void approve() {} }",
        )
        .unwrap();
        fs::write(
            root.join("acceptance-test/src/acceptance-test/java/demo/OrderAcceptanceTest.java"),
            "package demo; class OrderAcceptanceTest { void approve() {} }",
        )
        .unwrap();
        ...
        assert!(model.methods.iter().all(|method| {
            !method.file.contains("/test/")
                && !method.file.contains("integrationTest")
                && !method.file.contains("generated")
                && !method.file.ends_with("Test.java")
        }));
        let _ = fs::remove_dir_all(root);
    }
